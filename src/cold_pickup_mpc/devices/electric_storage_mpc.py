...
from cold_pickup_mpc.retrievers.electric_storage_retriever import ElectricStorageDataRetriever
...
class ElectricStorageMPC(DeviceMPC):
    """Electric storage class for the MPC."""
    ...
    def create_mpc_formulation(
        ...
        """
        Creates the optimization formulation of the electric storage device.
        Inputs:
        ...
        electric_storage_arrays = self._process_data_as_arrays(electric_storage_info, steps_horizon_k)
        ...
        charge_power = cvx.Variable((1, steps_horizon_k), nonneg=True, name="electric_storage_charge_power")
        discharge_power = cvx.Variable((1, steps_horizon_k), nonneg=True, name="electric_storage_discharge_power")
        residual_energy = cvx.Variable((1, steps_horizon_k + 1), nonneg=True, name="electric_storage_residual_energy")

        # Define optimization objective
        desired_state_array = desired_state * np.ones((1, steps_horizon_k))  # Constant desired SoC
        comfort_term = priority * cvx.sum(((desired_state_array - residual_energy[:, :-1]) / norm_factor) ** 2)
    ...
    def _process_data_as_arrays(
        ...
        initial_state = (
            electric_storage_info["initial_state"]["battery"] / 100 * electric_storage_arrays["energy_capacity"]
        )
        electric_storage_arrays["initial_state"] = initial_state

        # Minimum and maximum residual energy of the battery
        min_residual_energy = electric_storage_info["min_residual_energy"]["battery"] / 100 * energy_capacity
        max_residual_energy = electric_storage_info["max_residual_energy"]["battery"] / 100 * energy_capacity
        if initial_state > max_residual_energy:
            ...
            logger.warning("Setting max_residual_energy to total energy capacity of the battery.")
        ...
        electric_storage_arrays["priority"] = electric_storage_info["priority"]["battery"]

        # Final state of charge requirement of the battery
        electric_storage_arrays["final_soc_requirement"] = (
            electric_storage_info["final_soc_requirement"]["battery"] / 100 * electric_storage_arrays["energy_capacity"]
        )

        # Other variables
        electric_storage_arrays["power_capacity"] = electric_storage_info["power_capacity"]["battery"]
        electric_storage_arrays["decay_factor"] = electric_storage_info["decay_factor"]["battery"]
        electric_storage_arrays["charging_efficiency"] = electric_storage_info["charging_efficiency"]["battery"]
        electric_storage_arrays["discharging_efficiency"] = electric_storage_info["discharging_efficiency"]["battery"]
        electric_storage_arrays["norm_factor"] = electric_storage_info["energy_capacity"]["battery"]
