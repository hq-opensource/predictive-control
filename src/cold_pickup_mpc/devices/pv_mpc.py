...
class PhotovoltaicGeneratorMPC(DeviceMPC):
    ...
    def create_mpc_formulation(
        ...
        The PV forecast generated by pvlib here is completely INDEPENDENT from the 
        energy balance equation used by the CVXPY optimization solver. 
        
        Why? The non-controllable loads forecast is trained on historcial net-meter data, 
        which already intrinsically captures the physical generation of the PV panels. 
        If we explicitly fed this PV mathematical dispatch proxy back into the MPC 
        global energy balance, the solar generation would be double-counted.

        Therefore, this module only executes purely to generate a clean, theoretical 
        solar profile to be saved independently into InfluxDB for Grafana/UI visualization
        purposes. The returned 'dispatch' matrix for the PV is explicitly ignored during the model aggregation on the MPC.
        """
    ...
    def get_dispatch(
        ...
        # Generation is registered negative (-), consumption is positive (+)
        pv_production = -pv_system.results.ac.fillna(0) / 1000  # convert W to kW and negate
